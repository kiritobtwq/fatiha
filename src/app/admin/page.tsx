...
export default function AdminDashboardPage() {
  ...
  const handleLogout = async () => {
    ...
    window.location.href = '/alfatiha-secure-panel-2024x9k/login';
  };
  ...
  const handleArchiveContact = async (id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        alert(data.error || 'Ошибка при архивировании заявки');
      }
    } catch (error) {
      console.error('Error archiving contact:', error);
      alert('Ошибка при архивировании заявки');
    }
  };
  ...
  const handleTestDonation = async () => {
    ...
    const names = ['Ахмед', 'Фатима', 'Юсуф', 'Халид', 'Марьям', 'Ибрагим', 'Аноним'];
    const amount = amounts[Math.floor(Math.random() * amounts.length)];
    const name = names[Math.floor(Math.random() * names.length)];

    try {
      const res = await fetch('/api/admin/test-donation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount, donorName: name }),
      });
      if (res.ok) {
        loadData();
        alert(`Тестовый донат: ${name} — ${amount} ₽`);
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-4 lg:p-6 rounded-2xl shadow-sm border border-slate-200 sticky top-4 lg:top-24">
              ...
              <nav className="space-y-1 mb-6 lg:mb-8">
                {menuItems.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => setActiveTab(item.id)}
                    className={`flex items-center gap-3 w-full px-3 lg:px-4 py-3 rounded-xl text-xs lg:text-sm font-bold transition-all ${
                      activeTab === item.id 
              ...
              <button
                ...
                className="flex items-center gap-3 px-3 lg:px-4 py-3 rounded-xl text-xs lg:text-sm font-bold text-red-500 hover:bg-red-50 transition-colors w-full"
              >
                <LogOut size={18} />
          ...
          <div className="lg:col-span-4 space-y-8">
            {activeTab === 'dashboard' && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center justify-between">
                  <h1 className="font-display font-bold text-3xl text-slate-800">Обзор</h1>
                  <div className="flex gap-2">
                    <button onClick={handleTestDonation} className="text-sm font-bold text-primary bg-primary/10 px-4 py-2 rounded-lg hover:bg-primary/20 transition-colors">
                      + Тестовый донат
                    </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    ...
                    <div key={i} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      <div className={`text-2xl font-black mb-1 ${stat.color}`}>{isLoading ? '...' : stat.value}</div>
            ...
            {activeTab === 'donations' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center justify-between">
                  <h1 className="font-display font-bold text-3xl text-slate-800">Пожертвования</h1>
                  <div className="flex gap-3">
                    <div className="relative">
                      ...
                      <input 
                        type="text" 
                        placeholder="Поиск..." 
                        className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-primary w-64"
                      />
                    </div>
            ...
            {activeTab === 'contacts' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <h1 className="font-display font-bold text-3xl text-slate-800">Заявки</h1>
                  ...
                  {contacts.map((request) => (
                    <div key={request.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
                      ...
                      <div className="flex gap-3 pt-2">
                        ...
                        <button 
                          onClick={() => handleArchiveContact(request.id)}
                          className="px-4 py-2 bg-slate-100 text-slate-500 text-sm font-bold rounded-lg hover:bg-slate-200 transition-colors"
                        >
                          Архивировать
                        </button>
                        <button 
