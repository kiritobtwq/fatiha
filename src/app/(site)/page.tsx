...
export default function Home() {
  ...
  const getPrayerState = (prayerName: string, prayerTime: string): 'passed' | 'next' | 'upcoming' => {
    ...
    const dateStr = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}T${pad(h)}:${pad(m)}:00+05:00`;

    if (prayerName === 'Джума') {
      if (now.getDay() !== 5) return 'upcoming';
      const jumaTime = new Date(dateStr);
      const result = now > jumaTime ? 'passed' : 'upcoming';
      console.log('getPrayerState', { prayerName, prayerTime, prayerDate: jumaTime.toISOString(), now: now.toISOString(), result });
      return result;
    }
    ...
    console.log('getPrayerState', { prayerName, prayerTime, prayerDate: prayerDate.toISOString(), now: now.toISOString(), nextPrayer: nextPrayer?.name, result });
    return result;
  };
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-center min-h-screen pt-32 md:pt-44 pb-32">
            ...
            >
              <div className="bg-white rounded-3xl p-6 shadow-2xl" style={{ boxShadow: '0 30px 80px rgba(0, 0, 0, 0.3)' }}>
                ...
                <div className="flex p-1 gap-1 rounded-xl mb-4" style={{ backgroundColor: '#f3f4f6' }}>
